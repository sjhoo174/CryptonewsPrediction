        ...
        if page_data == []:
            ...
            for article in page_data:
                ...
                if date_time <= end_date and date_time >= start_date:
                    ...
                    r = requests.post(article_url)
                    soup = BeautifulSoup(r.text, 'html.parser')
                    article = soup.find("article", {"class": "cn-article-page"})
                    div = article.find("div", {"class" : "cn-content"})
                    paragraphs = div.find_all("p")
                    string = ""
                    for i in range(min(3, len(paragraphs))):
                        string += paragraphs[i].text

                    title_text = article_module.text
                    data['title'].append(title_text)
                    data['source'].append('cryptonews')
                    data['excerpt'].append(string)
                    data['article_url'].append(article_url)
                    data['source_id'].append('') # no article id avaialble
