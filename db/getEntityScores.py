...
from db.utils import preprocessTime as __preprocessTime
...
def toDatabase(df : pd.DataFrame, database: str = "db/sqlite.db") -> None: 
    ...
    for i, entity in enumerate(entityNames):
        ...
        if entry is None:
            ...
            entityId = entry[0]
            cur.execute('SELECT * FROM entity_scores WHERE (entity_id=?) AND (date_time=?)', (entityId, str(queryTime.iloc[i])))
            entry = cur.fetchone()

            if entry is None:
                cur.execute('INSERT INTO entity_scores (entity_score, date_time, entity_id) VALUES(?, ?, ?)', (entityScores.iloc[i], str(queryTime.iloc[i]), entityId))
...
def getEntityScore(entity: str, database : str = "db/sqlite.db") -> Union[None, list]:
    con = sqlite3.connect(database)
    cur = con.cursor()
    cur.execute('SELECT * FROM entities WHERE (name=?)', (entity,))
    entry = cur.fetchone()
...
def getEntityScoreAtTime(entity: str, time: str, database: str = "db/sqlite.db") -> Union[None, float]:
